...
export class Environment extends Identity {
	...
	/**
	 * Allow inter-system communication via the Environment.
	 * @path resolves to a System endpoint
	 */
	route(path, ...args) {
	 ...
	 * @path resolves to a Component endpoint
	 */
	next(path, ...args) {
	 ...
	 * @path resolves to a Spawner endpoint
	 */
	spawn(path, ...args) {
